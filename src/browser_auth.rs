...
pub async fn sign_in(
    ...
    let mut authorize = match reqwest::Url::parse(&format!(
        "{}/authorize",
        config.shared_auth_public_url
    )) {
        Ok(url) => url,
        Err(error) => {
            tracing::error!(%error, "invalid Shared Auth public URL");
            return error_json(StatusCode::INTERNAL_SERVER_ERROR, "invalid auth configuration");
        }
    };
    authorize
        .query_pairs_mut()
...
pub async fn callback(
    ...
    let redeem = match redeem {
        ...
        Err(error) => {
            ...
            return error_json(StatusCode::BAD_GATEWAY, "authentication upstream unavailable");
        }
    };
...
pub async fn invite_project(
    ...
    for org in viewer.orgs() {
        let projects = zed_orm_core::read::projects_for_org(db, org.id, &org.slug, true)
            .await
            .unwrap_or_default();
        if let Some(project) = projects.into_iter().find(|project| project.id == project_id) {
...
async fn refresh(
    ...
    let response = state
        ...
        .map_err(|error| {
            ...
            error_json(StatusCode::BAD_GATEWAY, "authentication upstream unavailable")
        })?;
...
async fn delegate(
    ...
    Ok(decode_json::<DelegateResponse>(response, "token delegation")
        .await?
        .access_token)
}
...
async fn decode_json<T: for<'de> Deserialize<'de>>(
    ...
    response.json::<T>().await.map_err(|error| {
        ...
        error_json(StatusCode::BAD_GATEWAY, "authentication response was invalid")
    })
}
...
mod tests {
    ...
    fn return_targets_are_local_paths_only() {
        assert_eq!(sanitize_return_to("/dashboard/acme?tab=1"), "/dashboard/acme?tab=1");
